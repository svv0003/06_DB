...
INSERT INTO customers
		VALUES (9, '조민수', 'minsoo.jo@gmail.com', 'min123!', '010-2468-1357', '대전시 유성구 대학로 99', now());
...
SELECT * FROM stores;
INSERT INTO stores
		VALUES (null, '맘스터치 대학로점', '9', '대전시 유성구 대학로 100', '042-123-4567', '수제버거 전문점', '12000', '2500', '4.6', '매일 10:30 - 22:00', now());
...
SELECT * FROM customers;
INSERT INTO customers
		VALUES (null, '이수진', 'suejin@naver.com', 'sue9876', null, '서울시 영등포구 당산로 200', now()),
        (null, '김태원', 'taewon88@gmail.com', 'tae4567', null, '부산시 부산진구 서면로 150', now()),
        (null, '박소희', 'sohee@daum.net', 'hope123', null, '인천시 연수구 송도대로 300', now());
...
SELECT * FROM orders;
insert into orders
values (null, 2, 1, null, 'Pending', '26000', '서울시 서초구 서초대로 456', '양념 소스 추가 부탁드려요');

-- 문제 17
-- 위에서 추가한 주문(가장 최근 주문)의 상태를 'Cooking'으로 변경하시오.
UPDATE orders
SET order_status = 'Cooking'
WHERE delivery_address = '서울시 서초구 서초대로 456';
...
SELECT * FROM menus;
insert into customers
		values (null, '신미래', 'future@email.com', 'future2024', '010-2024-2025', '경기도 성남시 분당구 판교로 500', now());
insert into orders
...
select * FROM orders;
update orders
set order_status = 'VIP_Delivered'
where order_status = 'Delivered'
AND total_price > 30000;

...
SELECT professor_no 교수코드, professor_name 교수명, department_no 소속학과
FROM professor;
...
SELECT NOW() + interval 7 Day '7일 후', NOW() + interval 30 Day '30일 후', NOW() + interval 365 Day '365일 후';
...
SELECT point + 0.5 '조정 성적'
FROM grade;

-- 문제 10
-- DEPARTMENT 테이블에서 정원에서 5명을 뺀 값을 '실제모집인원'으로 조회하시오.
SELECT capacity-5 '실제 모집 정원'
FROM department;

...
/*
SQL 작동 확인하는 방법
대표적으로 SELECT VERSION(); 작성 후
Ctrl (Command) + Enter 입력 시 버전 나오는 지 확인한다.
...
/*
CREATE DATABASE		DB를 생성할 것인데
IF NOT EXISTS		만약 오른쪽에 작성한 DB 명칭이 존재하지 않는다면
employee_management	employee_management 명칭의 DB를 생성한다.
;					한 줄 작성 완료 표시
*/

USE employee_management;
/*
USE employee_management;	개발자나 관리자가 사용할 DB 명칭을 선택해서 사용하겠다.

이후 실행될 모든 SQL 명령어는 employee_management DB에서 동작하고 작동하도록 설정한다.
PM 직급이나 팀장급 사람들만 주로 사용하는 명령어이다.
*/

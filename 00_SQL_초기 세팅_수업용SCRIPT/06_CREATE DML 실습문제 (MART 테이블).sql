...
CREATE TABLE MART (
	...
	phone VARCHAR(20),	# INT는 010 의 맨 앞에 있는 0이 자동을 사라진다.
						# 주민등록번호, 연락처 등 문자열로 설정하기.
    open_time TIME ,
    close_time TIME ,
...
INSERT INTO mart
		VALUES (null, '이마트 강남점', '서울시 강남구 테헤란로 123', '02-123-4567', '08:00', '24:00', FALSE, now(), now());
...
INSERT INTO mart (mart_name, location, phone, open_time, close_time)
		VALUES ('롯데마트 잠실점', '서울시 송파구 올림픽로 456', '02-456-7890', '09:00', '23:00');

-- 문제 2-3: DEFAULT 값 활용하여 삽입
-- CU 편의점 역삼점, 서울시 강남구 역삼로 789, 02-789-0123, 24시간 운영
INSERT INTO mart (mart_name, location, phone, is_24hour)
		VALUES ('CU 편의점 역삼점', '서울시 강남구 역삼로 789', '02-789-0123', 1);
...
-- DATE_SUB (NOW(), INTERVAL 10 MINUTE); 이용해서 등록된 지 10분 이내의 마트만 조회하기.
SELECT *
FROM mart
WHERE create_at >= DATE_SUB(NOW(), INTERVAL 10 MINUTE);
...
INSERT INTO projects
		VALUES (null, '웹사이트 리뉴얼', '1', 50000000, '2024-01-01', '2024-06-30', 'IN_PROGRESS', null, now()),
				(null, '모바일 앱 개발', '3', 80000000, '2024-02-15', '2024-12-31', 'PLANNING', null, now());
...
-- ========================================
-- 3. INSERT 서브쿼리
-- MySQL 특징 - 같은 테이블을 조작함과 동시에 조회할 때 발생하는 문제!
-- 			   Error 1094: 동시에 할 수 없다.
-- 			 -> 조회할 데이터를 INSERT 내에서만 사용하는 파생 테이블 형태로 만들고,
-- 			 -> INSERT 데이터를 저장하는 형식을 사용한다.
-- ========================================
...
INSERT INTO employees
		VALUES (null, '홍길동', 'IT', '사원', (SELECT avg(salary) FROM employees 평균급여 WHERE department = 'IT') ,'2024-09-01', 'hong.gd@company.com', null, null, now(), null);
		-- VALUES (null, '홍길동', 'IT', '사원', (SELECT avg(salary) FROM (SELECT avg(salary) FROM employees WHERE department = 'IT') 평균급여) ,'2024-09-01', 'hong.gd@company.com', null, null, now(), null);

DROP TABLE projects;

-- 변수 사용하기.
-- SET @변수명 = (쿼리) ;
SET @평균급여 = (SELECT avg(salary) FROM employees WHERE department = 'IT');
SELECT avg(salary) FROM employees WHERE department = 'IT';

INSERT INTO employees
		VALUES (null, '홍길동', 'IT', '사원', @평균급여 ,'2024-09-01', 'hong.gd@company.com', null, null, now(), null);
        
DELETE
FROM employees
where email = 'hong.gd@company.com';
...
INSERT INTO projects
		VALUES (null, 'AI 챗봇 개발', @직원아이디, 120000000, '2024-10-01', '2025-03-31', 'PLANNING', null, now());

DELETE
FROM projects
where project_name = 'AI 챗봇 개발';
...
*/
SET @직원입사일 = (SELECT max(hire_date) FROM employees);
SELECT * FROM employees;
INSERT INTO employees
		VALUES (null, '윤서연', 'HR', '사원', 3800000, @직원입사일, 'yoon.sy@company.com', null, null, now(), null);
...
SET @평균급여2 = (SELECT avg(salary) FROM employees);
UPDATE employees
SET salary = @평균급여2
...
UPDATE employees E
JOIN (SELECT department, max(salary) 최고급여 FROM employees group by department) M
ON E.department = M.department AND E.salary = M.최고급여
SET position = '팀팀장';
...
UPDATE employees E
JOIN (SELECT manager_id FROM projects WHERE status = 'completed') M
ON M.manager_id = E.emp_id
set salary = salary * 1.2;

UPDATE employees E
SET salary = salary * 1.2
WHERE EXISTS (SELECT 1 FROM projects WHERE status = 'completed' AND manager_id = E.emp_id);
